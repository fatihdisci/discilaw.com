...
function Button({ children, className = '', variant = 'primary', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'ghost' | 'danger' }) {
  ...
  return (
    <button
      className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition-colors focus-visible:ring-4 focus-visible:ring-[var(--brand-soft)] disabled:cursor-not-allowed disabled:opacity-50 ${variants[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
...
const inputClass = 'box-border min-h-12 w-full rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] px-3.5 py-2.5 text-sm text-[var(--ink-strong)] shadow-[inset_0_1px_1px_rgba(45,55,38,0.03)] outline-none transition-colors placeholder:text-[var(--ink-faint)] focus:border-[var(--brand-deep)] focus:ring-4 focus:ring-[var(--brand-soft)]';
const textareaClass = `${inputClass} resize-y overflow-x-hidden leading-6`;
...
function EmptyState({ icon: Icon = Files, title, description, action }: { icon?: typeof Files; title: string; description: string; action?: ReactNode }) {
  return (
    <div className="grid place-items-center rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-elevated)] px-6 py-14 text-center shadow-[var(--shadow-sm)]">
...
function DocumentList({ client, documents, setNotice }: { client: SupabaseClient; documents: DocumentRecord[]; setNotice: (notice: Notice) => void }) {
  if (!documents.length) return <p className="rounded-xl border border-[var(--border-soft)] bg-[var(--bg-elevated)] px-4 py-6 text-center text-sm text-[var(--ink-muted)]">Henüz paylaşılmış belge yok.</p>;
  return (
...
function SubmissionForm({ client, userId, onCreated }: { client: SupabaseClient; userId: string; onCreated: () => Promise<void> }) {
  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="grid max-w-3xl gap-5 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-elevated)] p-5 shadow-[var(--shadow-sm)] sm:p-7">
        ...
        <Field label="Kısa açıklama" hint="Kişisel veya hassas bilgileri yalnızca işlem için gerekliyse ekleyin."><textarea className={`${textareaClass} min-h-28`} maxLength={5000} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Evrakların ne için gönderildiğini kısaca açıklayın." /></Field>
...
function AdminCaseDetail({ client, adminId, caseRecord, documents, onBack, onChanged, setNotice }: { client: SupabaseClient; adminId: string; caseRecord: CaseRecord; documents: DocumentRecord[]; onBack: () => void; onChanged: () => Promise<void>; setNotice: (notice: Notice) => void }) {
  ...
  return (
    <div className="mt-7"><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 text-sm text-[var(--brand-deep)]"><ArrowLeft className="size-4" />Dosya listesine dön</button><h2 className="font-display text-3xl font-medium">{caseRecord.title}</h2>
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <form onSubmit={saveCase} className="grid content-start gap-4 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-elevated)] p-5 shadow-[var(--shadow-sm)]"><h3 className="font-display text-xl font-medium">Dosya Bilgileri</h3><Field label="Durum"><select className={inputClass} value={status} onChange={(e) => setStatus(e.target.value)}>{caseStatuses.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Müvekkile açık kısa açıklama"><textarea className={`${textareaClass} min-h-28`} maxLength={5000} value={summary} onChange={(e) => setSummary(e.target.value)} /></Field><Button type="submit" disabled={busy}>Bilgileri Kaydet</Button></form>
        <form onSubmit={addUpdate} className="grid content-start gap-4 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-elevated)] p-5 shadow-[var(--shadow-sm)]"><div><h3 className="font-display text-xl font-medium">Müvekkile Açık Güncelleme</h3><p className="mt-1 text-xs text-[var(--danger)]">Bu alan müvekkil tarafından görülebilir. İç not yazmayın.</p></div><Field label="Başlık"><input className={inputClass} required maxLength={160} value={updateTitle} onChange={(e) => setUpdateTitle(e.target.value)} /></Field><Field label="Açıklama"><textarea className={`${textareaClass} min-h-28`} required maxLength={5000} value={updateBody} onChange={(e) => setUpdateBody(e.target.value)} /></Field><Button type="submit" disabled={busy}>Güncellemeyi Yayımla</Button></form>
